...
function initQueryLogDurationChart(data) {
    ...
    $('#querylog-duration-chart').parent().block();

    loadViaAjax(query_duration_and_count_data_url, data, 'json', null, null, null, function (result) {
        rawChartData = result;
        ...
        setTimeout(function () {
            $('#querylog-duration-chart').parent().unblock();
        }, 500);
    });
...
function initQueryLogCountChart(data) {
    ...
    $('#querylog-count-chart').parent().block();
    ...
    loadViaAjax(query_duration_and_count_data_url, data, 'json', null, null, null, function (result) {
        ...
        makeChart(getNormalChartGraphsArray(result), pd);
        setTimeout(function () {
            $('#querylog-count-chart').parent().unblock();
        }, 500);
    });
...
function initHourlyQueriesChart(data) {
    ...
    $('#hourly-queries-chart').parent().block();
    ...
    loadViaAjax(query_hourly_data_url, data, 'json', null, null, null, function (result) {
        rawChartData = result;
        //var sequencer = rawChartData[rawChartData.length - 1];
        var pd = pivotHourlyChartData(result);
        makeChart(getGraphsArray2(result), pd);
        setTimeout(function () {
            $('#hourly-queries-chart').parent().unblock();
        }, 500);
    });
...
function initHourlyComparisonQueriesChart(data) {
    ...
    $('#hourly-queries-comparison-chart').parent().block();
    ...
    loadViaAjax(query_hourly_data_url, data, 'json', null, null, null, function (result_1) {
        ...
        if (comparisonExists) {
            ...
            loadViaAjax('/dashboard/ql/hourlyavgchartdata', data, 'json', null, null, null, function (result_2) {
                ...
                makeChart(graphs, pd);
                setTimeout(function () {
                    $('#hourly-queries-comparison-chart').parent().unblock();
                }, 500);
...
$(function () {
    ...
    $('#timespan-filter').on('change', function (e) {
