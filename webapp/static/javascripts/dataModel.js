    ...
    function buildLinks() {
        ...
        for(var i=0; i<this.nodes.length; i++){
            ...
            for(var j=0; j<this.nodes[i].baseTable.joins.length; j++) {
                var arr = this.nodes[i].baseTable.joins[j];

                if(arr['Join Usage Score'] <= parseFloat(this.filters.greaterThanJoin)){
                    ++count
                }
                // alert(this.nodes[i].baseTable.joins[j])
            }
            if(count == this.nodes[i].baseTable.joins.length) {
                this.nodes[i].baseTable.joins.length = 0;
            }
        }
